...
drand publishes cryptographically verifiable randomness, but Worldloom is an artwork rather than a beacon verifier. This release relies only on HTTPS, pinned chain metadata, and structural response validation. It does not verify the BLS signature, recompute chain identity, or compare relay consensus, and it must not describe the derived digest as verified, authenticated, or cryptographically validated randomness. User-facing copy calls it a “drand Quicknet round” or a pulse derived from a structurally accepted response.
...
- **Solana:** persist the last committed slot atomically with its completed summary and checkpoint. Newer accepted slots remain only in worker-local current or pending state. A socket reconnect in the same process retains that working continuity; a process restart initializes from the committed checkpoint. After reconnect, subscribe to current progress and use the first returned slot to establish the new live edge. If a committed or worker-local previous slot is known, a forward jump contributes to the next summary's gap count; it is never expanded into fabricated slots. A duplicate or backward first position is dropped while the adapter waits for newer progress. Receipt alone never advances durable continuity, and a failed commit leaves both aggregate state and checkpoint continuity unchanged.
