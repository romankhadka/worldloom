...
The struct may contain only UTC window time, counters, capped hash sets, and `truncated`; derive `Inspect` to omit every hash set. Never retain or expose collector, peer, peer ASN, message ID, next hop, prefix, path, community, raw bytes, or the source frame in state, output, logs, telemetry, PubSub, or errors.
