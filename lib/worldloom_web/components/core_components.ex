defmodule WorldloomWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: WorldloomWeb.Gettext
  ...
  def flash(assigns) do
    ...
    >
      <div class={[
        ...
        <button
          type="button"
          class="group self-start cursor-pointer text-current"
          aria-label={gettext("close")}
        >
          <.icon name="hero-x-mark" class="size-5 opacity-50 transition group-hover:opacity-100" />
        </button>
      </div>
    </div>
    """
  ...
  def button(%{rest: rest} = assigns) do
    ...
    assigns =
      assign_new(assigns, :class, fn ->
        [
          "inline-flex min-h-10 items-center justify-center border px-4 py-2 text-xs font-semibold uppercase tracking-[0.16em] transition focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-40",
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(WorldloomWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(WorldloomWeb.Gettext, "errors", msg, opts)
    end
  end
